...
get_stats() {
  if is_pool_mode; then
    # No local node in pool mode — report the pool as the work source instead
    BITCOIN_STATUS="N/A"   # unused in pool block
    BLOCKS="N/A"; HEADERS="N/A"; SYNC_PERCENT="N/A"
  else
    # Bitcoin Core status
  ...
  fi  # end work-source (solo node) status
    ...
    if pgrep -f "cpuminer" > /dev/null 2>&1; then
        ...
        if [ -n "$MINER_API" ]; then
            ...
            # Numeric > 0 test: cpuminer reports "0.00" while syncing, which a
            # plain string "!= 0" check wrongly treats as active mining.
            if [ -n "$HASHRATE_KHS" ] && awk "BEGIN{exit !(${HASHRATE_KHS}+0 > 0)}"; then
    ...
    # Network difficulty (solo mode only — needs the local node)
    if is_pool_mode; then
        DIFFICULTY="N/A"
    ...
    if [ -n "$BITCOIN_MINING_ADDRESS" ] && [ "$BITCOIN_MINING_ADDRESS" != "YOUR_BITCOIN_ADDRESS_HERE" ]; then
        # Try local wallet first (solo only — pool mode has no node/wallet)
